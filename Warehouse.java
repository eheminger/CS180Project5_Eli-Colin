...
public class Warehouse {
    ...
    public static int findMeanValue(ArrayList<Package> packageList) {
        ...
        int maxNum = 0;
        int iterator;
        for (Package temp: packageList) {
            iterator = 0;
            for (Package temp2: packageList) {
                if (temp.getDestination().getZipCode() == temp2.getDestination().getZipCode()) {
                    iterator++;
                }
            }
            if (iterator > maxNum) {
                maxNum = iterator;
                maxZip = temp.getDestination().getZipCode();
            }
        }
        return maxZip;
    }
